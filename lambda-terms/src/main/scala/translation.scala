...
def deleteNames(t: named.Term, context: Array[named.Var], nestedλ: Int = 0, bVtoλMap: Map[named.Var, Int] = Map()): nameless.Term =
  t match
    case named.Var(x) => 
      if bVtoλMap.contains(x) then
        // if the variable is bounded its index is the number of abstractions between its occurence and the abstraction where it is bounded
        nameless.Var(nestedλ - bVtoλMap(x)) 
      else
        // if not then its index is its de Bruijn index + the number of abstractions
        nameless.Var(context.indexOf(x) + nestedλ)
    case named.Apl(m, n) => 
...
def newVar(context: Array[named.Var], boundedVars: Map[Int, named.Var]): named.Var =
...
def addNames(t: nameless.Term, context: Array[named.Var], nestedλ: Int = 0, boundedVars: Map[Int, named.Var] = Map()): named.Term =
  t match
    case nameless.Var(n) =>
      if n >= nestedλ then
