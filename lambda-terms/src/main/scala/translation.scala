...
def deleteNames(t: named.Term, context: Vector[named.Var], nestedλ: Int = 0, bVtoλMap: Map[named.Var, Int] = Map()): nameless.Term =
  t match
    case named.Var(x) => 
...
def addNames(t: nameless.Term, context: Vector[named.Var], nestedλ: Int = 0, boundedVars: Map[Int, named.Var] = Map()): named.Term =
  t match
    case nameless.Var(n) =>
      if n >= nestedλ then
