...
def normalReduction(m: Term): Option[Term] = m match
  case λ(x, n) =>
    normalReduction(n).map(y =>
      λ(x, y)
    ) // if n is reducible then λ(x, nR(n)) else None (because λ(x, n) will also be non reducible)
  ...
  case Apl(p, q) =>
    ...
    reducedP match
      case Some(t) => Apl(t, q)
      case None =>
        normalReduction(q).map(y =>
          Apl(p, y)
        ) // if nR(q) is term then Apl(p, nR(q)) else None (because neither p nor q are reducible and p is not λ(x,p))
  case _ => None
